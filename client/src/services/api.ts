...
class ApiService {
  ...
  async getWallets(): Promise<Wallet[]> {
    const res = await this.request<{ payer: Wallet; payout: Wallet }>(
      "/wallets",
    );
    return [res.payer, res.payout].filter(Boolean);
  }

  async getPinStatus(): Promise<{
    has_pin: boolean;
    locked_until?: string;
  ...
  async setPin(pin: string): Promise<{ ok: boolean }> {
    return this.request("/wallets/pin/set", {
      method: "POST",
  ...
  async changePin(
    ...
    return this.request("/wallets/pin/change", {
      method: "POST",
  ...
  async createDeposit(
    role: "payer" | "payout",
    amount: number,
  ): Promise<{ id: string; instructions?: any }> {
    return this.request<{ id: string; instructions?: any }>(
      "/wallets/deposits",
      {
        method: "POST",
  ...
  async createWithdrawal(
    ...
    to: string,
    pin?: string,
  ): Promise<{ id: string; tx_hash?: string }> {
    return this.request<{ id: string; tx_hash?: string }>(
      "/wallets/withdrawals",
      {
        method: "POST",
  ...
  async getProviderResources(limit = 50): Promise<Resource[]> {
    const res = await this.request<{ items: Resource[] }>(
  ...
  async getProviderEarnings(days: number = 30): Promise<EarningsData> {
    return this.request<EarningsData>(
      `/analytics/provider/earnings?days=${days}`,
    );
  }
  ...
  async getReceipts(limit = 20): Promise<Receipt[]> {
    const res = await this.request<{ items: Receipt[] }>(
  ...
  async getCatalogResources(params?: {
    limit?: number;
    category?: string;
    q?: string;
    ...
    const res = await this.request<{ items: CatalogResource[] }>(
      `/catalog/resources?${qs.toString()}`,
    );
    return res.items;
  }

  async getCatalogResource(id: string): Promise<CatalogResource> {
    return this.request<CatalogResource>(
      `/catalog/resources/${encodeURIComponent(id)}`,
    );
  }
  ...
  async getConsumerActivity(limit = 5): Promise<ConsumerActivityItem[]> {
    const res = await this.request<{ items: ConsumerActivityItem[] }>(
      `/analytics/consumer/activity?limit=${limit}`,
    );
    return res.items;
  }

  async getConsumerTopAgents(limit = 5): Promise<TopAgentStat[]> {
    const res = await this.request<{ items: TopAgentStat[] }>(
      `/analytics/consumer/top-agents?limit=${limit}`,
    );
    return res.items;
  }

  async getConsumerTopSources(limit = 5): Promise<TopSourceStat[]> {
    const res = await this.request<{ items: TopSourceStat[] }>(
      `/analytics/consumer/top-sources?limit=${limit}`,
    );
    return res.items;
  }
  ...
  async updateConnector(
    id: string,
    payload: Partial<ConnectorPayload> & { status?: "active" | "disabled" },
  ): Promise<void> {
  ...
  async initSiteVerification(
    domain: string,
    method: "dns" | "file",
  ): Promise<SiteVerificationInitResponse> {
  ...
  async checkSiteVerification(
    domain: string,
    method: "dns" | "file",
    token: string,
  ): Promise<SiteVerificationCheckResponse> {
  ...
  async getResourcePerformance(limit = 10): Promise<ResourcePerformance[]> {
    const res = await this.request<{ items: ResourcePerformance[] }>(
      `/analytics/provider/resources/performance?limit=${limit}`,
    );
    return res.items;
  }
  ...
  async getProviderRequests(
    ...
    const res = await this.request<{ requests: ProviderRequest[] }>(
      `/providers/requests?${params.toString()}`,
    );
    return res.requests;
  }
  ...
  async getWithdrawals(limit = 50): Promise<Withdrawal[]> {
    const res = await this.request<{ withdrawals: Withdrawal[] }>(
      `/wallets/withdrawals?limit=${limit}`,
    );
    return res.withdrawals;
  }

  async getDeposits(limit = 50): Promise<Deposit[]> {
    const res = await this.request<{ deposits: Deposit[] }>(
      `/wallets/deposits?limit=${limit}`,
    );
    return res.deposits;
  }

  async uploadEncryptedToWalrus(
    file: File,
    ...
    const res = await this.request<{
      ...
      method: "POST",
      body: JSON.stringify({
        encrypted_object_b64: b64,
        epochs: opts?.epochs ?? 1,
        deletable: opts?.deletable ?? true,
      }),
    });
    return {
      walrus_blob_id: res.walrus_blob_id,
      walrus_blob_object_id: res.walrus_blob_object_id,
      size_bytes: file.size,
      cipher_meta: { algo: "seal-ibe-aes256gcm", size_bytes: file.size },
    };
  }
  ...
  async getMarkets(limit = 24): Promise<Market[]> {
    const res = await this.request<{ items: Market[] }>(
      `/markets?limit=${limit}`,
    );
    return res.items;
  }
  ...
  async getWalrusQuote(params: {
    ...
    epochs: number;
    wal_est: number;
    wal_breakdown?: { storage: number; write: number };
    sui_est: number | null;
  }> {
    return this.request(`/walrus/quote`, {
      method: "POST",
      body: JSON.stringify(params),
    });
  }

  async getWalrusExtendQuote(params: {
    size_bytes: number;
    add_epochs: number;
  }): Promise<{
    encoded_bytes?: number;
    ...
    return this.request(`/walrus/extend-quote`, {
      method: "POST",
      body: JSON.stringify(params),
    });
  }
  ...
  async extendWalrusStorage(params: {
    ...
    return this.request(`/walrus/extend`, {
      method: "POST",
      body: JSON.stringify(params),
    });
  }
