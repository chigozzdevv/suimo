...
import { Link, Search, Wallet, CheckCircle, Globe, Upload, Plug, Settings2, Shield, DollarSign } from 'lucide-react'
    ...
    <section id="how">
      ...
      >
        ...
        <p className="mt-3 text-base md:text-lg text-fog max-w-2xl mx-auto">
          Connect via MCP, discover licensed resources, then pay & fetch — powered by Walrus and SUI.
        </p>
      </motion.div>

      {/* Agents */}
      <div className="text-center text-sm uppercase tracking-widest text-fog mb-1">For AI Agents</div>
      <div className="mx-auto mb-4 h-px w-24 bg-white/10" />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        <StepCard step={1} title="Connect via MCP" description="Connect Suimo's MCP to your agent (Claude, ChatGPT, any OAuth-capable client) and authenticate with OAuth 2.0 (PKCE)">
          <div className="min-h-[190px] md:min-h-[220px] rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex-1 flex flex-col justify-start">
            <div className="text-xs uppercase tracking-wider text-fog mb-3">Endpoint</div>
        ...
        <StepCard step={2} title="Discover resources" description="Type your query; agent calls discover_resources to find licensed sources with details; you select the one you want">
          <div className="min-h-[190px] md:min-h-[220px] rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex-1 flex flex-col justify-start">
        ...
        <StepCard step={3} title="Pay & Fetch" description="Pay with your custodial balance in SUI USDC; receive a signed cryptographic receipt and your agent receives the data to work with">
          <div className="min-h-[190px] md:min-h-[220px] rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex-1 flex flex-col justify-start">
            <div className="text-xs font-mono text-parchment/80 mb-2">POST /v1/fetch</div>
      ...
      {/* Providers */}
      <div className="mt-10 text-center text-sm uppercase tracking-widest text-fog mb-1">For Data Providers</div>
      <div className="mx-auto mb-4 h-px w-24 bg-white/10" />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        <StepCard step={4} title="Sign up & verify" description="Add your domain and verify via DNS TXT, or upload a file">
        ...
        <StepCard step={5} title="Auth & pricing" description="Add an auth connector, set pricing & access; or use Suimo's internal connector">
          <div className="min-h-[190px] md:min-h-[220px] rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex-1">
            <div className="grid grid-cols-1 auto-rows-min items-start gap-3">
        ...
        <StepCard step={6} title="Publish & payouts" description="Publish resources. Usage credits accrue and settle to your payout wallet">
          <div className="min-h-[190px] md:min-h-[220px] rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex-1">
            <div className="grid grid-cols-1 auto-rows-min items-start gap-3">
...
function StepCard({
  ...
  return (
    ...
    >
      <div className="mb-2 text-xs uppercase tracking-widest text-fog">Step {String(step).padStart(2, '0')}</div>
...
function ResultRow({ name, badge }: { name: string; badge: string }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2">
      ...
      <span className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5 text-xs text-fog">{badge}</span>
    </div>
  )
}

function Pill({ children, icon: Icon, color }: { children: ReactNode; icon?: any; color?: 'blue' | 'purple' | 'cyan' | 'orange' | 'green' }) {
  ...
  const tint = color ? `bg-gradient-to-br ${styles[color]} text-parchment` : 'bg-white/5 text-fog'
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs ${tint} border-white/10`}>
      {Icon && <Icon className="h-3.5 w-3.5 text-sand" />} {children}
    </span>
  )
}

function ColorTile({ icon: Icon, label, color }: { icon: any; label: string; color: 'blue' | 'purple' | 'cyan' | 'orange' | 'green' }) {
  const styles: Record<string, string> = {
    blue: 'from-blue-500/20 to-blue-600/10 border-blue-500/30',
    purple: 'from-purple-500/20 to-purple-600/10 border-purple-500/30',
    cyan: 'from-cyan-500/20 to-cyan-600/10 border-cyan-500/30',
  ...
  return (
    <div className={`flex items-center gap-2 rounded-xl border bg-gradient-to-br ${styles[color]} px-3 py-2 text-sm text-parchment shadow-lg shadow-black/10`}>
