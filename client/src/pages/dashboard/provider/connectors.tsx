...
export function ConnectorsPage() {
  ...
  const [formState, setFormState] =
    useState<ConnectorFormState>(defaultConnectorForm);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (formState.type === "api_key") {
        ...
        await api.createConnector({
          type: "jwt",
          config: { header: formState.header, token: formState.token },
        });
      } else if (formState.type === "oauth") {
        await api.createConnector({
          type: "oauth",
          config: { access_token: formState.access_token },
        });
      } else {
  ...
  const toggleConnectorStatus = async (connector: Connector) => {
    try {
      await api.updateConnector(connector.id, {
        status: connector.status === "disabled" ? "active" : "disabled",
      });
      await loadConnectors();
    } catch (err: any) {
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm text-fog">
            Store API keys, OAuth tokens, or internal credentials.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <StatCard
          ...
          helper="Across all types"
        />
        <StatCard
          label="Active"
          value={active.toString()}
          helper="Ready for crawls"
        />
        <StatCard
          label="Coverage"
          value={`${coverage}`}
          helper="Unique auth types"
        />
      </div>
        ...
        <CardContent className="space-y-4 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-fog">
                Rotated daily and encrypted at rest.
              </p>
            </div>
          </div>
          {connectors.length === 0 ? (
            ...
            <div className="space-y-3">
              {connectors.map((connector) => (
                ...
                >
                  <div>
                    <p className="text-sm font-semibold capitalize text-parchment">
                      {connector.type}
                    </p>
                    <p className="text-xs text-fog">
                      {connector.status === "disabled" ? "Disabled" : "Active"}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button
                      variant="ghost"
                      className="gap-2 border border-white/10 px-3 py-1 text-xs text-fog hover:text-parchment"
                      onClick={() => toggleConnectorStatus(connector)}
                    >
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              <Field label="Header">
                <input
                  ...
                  onChange={(e) =>
                    setFormState({ ...formState, header: e.target.value })
                  }
              ...
              <Field label="Scheme">
                <input
                  ...
                  onChange={(e) =>
                    setFormState({ ...formState, scheme: e.target.value })
                  }
                  className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-parchment focus:border-white/40 focus:outline-none"
                />
              </Field>
              <Field label="Token">
                <textarea
                  ...
                  onChange={(e) =>
                    setFormState({ ...formState, token: e.target.value })
                  }
          ...
          {formState.type === "internal" && (
            <p className="text-sm text-fog">
              Internal connectors rely on Polycrawl-managed secrets. No
