...
export function WalletPage() {
  ...
  const [setPinStatusMsg, setSetPinStatusMsg] = useState<{
    tone: "success" | "error";
    message: string;
  } | null>(null);
  ...
  const handleWithdraw = async () => {
    ...
    if (pinStatus?.has_pin) {
      if (!withdrawPin) {
        setWithdrawStatus({
          tone: "error",
          message: "Enter your withdrawal PIN.",
        });
        return;
      }
      if (
        pinStatus.locked_until &&
        new Date(pinStatus.locked_until).getTime() > Date.now()
      ) {
        setWithdrawStatus({
          tone: "error",
    ...
    try {
      const res = await api.createWithdrawal(
        ...
        pinStatus?.has_pin ? withdrawPin : undefined,
      );
      setWithdrawStatus({
        tone: "success",
        message: `Withdrawal ${res.id} sent. Check ${withdrawAddress.trim()} after a few seconds.`,
      });
      setWithdrawAmount("");
      setWithdrawAddress("");
      setWithdrawPin("");
      ...
      const friendly = msg.includes("PIN_REQUIRED")
        ? "Withdrawal PIN is required."
        : msg.includes("PIN_INVALID")
          ? "Invalid PIN. Please try again."
          : msg.includes("PIN_LOCKED")
            ? "Too many attempts. PIN is locked temporarily."
            : msg.includes("PIN_NOT_SET")
              ? "Please set a withdrawal PIN to continue."
    ...
    <>
      <div className="space-y-6">
        <div className="flex items-center gap-2 text-xs text-fog">
          ...
          <span>Network:</span>
          <span
            className={`rounded-full px-2 py-0.5 ${network === "Testnet" ? "bg-white/10 text-parchment" : "bg-white/5 text-parchment"}`}
          >
            {network}
          </span>
        </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm pt-2 border-t border-white/10">
                    ...
                    <span
                      className={`font-medium ${payerWallet?.status === "active" ? "text-parchment" : "text-ember"}`}
                    >
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm pt-2 border-t border-white/10">
                    <span className="text-fog">Status</span>
                    <span
                      className={`font-medium ${payoutWallet?.status === "active" ? "text-parchment" : "text-ember"}`}
                    >
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-parchment font-medium">
            Fund your wallets on {network}
          </p>

          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="mb-3 text-xs uppercase tracking-wider text-fog/70">
              Your Payer Wallet Address
            </div>
            <div className="flex items-center gap-2">
              <p className="flex-1 break-all font-mono text-sm text-parchment">
                {payerWallet?.address || "Address unavailable"}
              </p>
              <button
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <ol className="space-y-3 text-sm text-parchment/90">
              <li className="flex gap-3">
                ...
                <span>
                  ...
                  <a
                    ...
                    className="underline hover:text-parchment font-medium"
                  >
                    faucet.sui.io
                  </a>
                </span>
              ...
              <li className="flex gap-3">
                ...
                <span>
                  Select <strong>Sui {network}</strong>
                </span>
              </li>
              <li className="flex gap-3">
          ...
          <p className="text-xs text-fog/70">
            Balances update after confirmation (~1 minute)
          </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-fog">
            Send payout wallet funds to a SUI address (testnet). You can request
            up to ${MAX_REQUEST_AMOUNT.toLocaleString()} per withdrawal.
          </p>
              ...
              {pinStatus?.locked_until &&
                new Date(pinStatus.locked_until).getTime() > Date.now() && (
                  <div className="text-xs text-ember">
                    PIN locked until{" "}
                    {new Date(pinStatus.locked_until).toLocaleTimeString()}
                  </div>
                )}
            </label>
          ) : (
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-fog">
            Set a 4–6 digit PIN to authorize withdrawals.
          </p>
          <label className="space-y-1 text-sm text-fog">
            <span>New PIN</span>
            <input
              ...
              className="w-full rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-parchment"
            />
          </label>
          <label className="space-y-1 text-sm text-fog">
          ...
          <Button
            onClick={async () => {
              ...
              try {
                ...
                setSetPinStatusMsg({
                  tone: "error",
                  message: e?.message || "Unable to set PIN",
                });
              }
            }}
            className="w-full"
          ...
          {setPinStatusMsg && (
            <div
              className={`rounded-xl border px-3 py-2 text-sm ${setPinStatusMsg.tone === "success" ? "border-sand/40 bg-sand/10 text-sand" : "border-ember/40 bg-ember/10 text-ember"}`}
            >
              {setPinStatusMsg.message}
            </div>
          )}
        </div>
      </Modal>
