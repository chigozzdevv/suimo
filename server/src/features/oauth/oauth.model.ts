...
export async function insertAuthorizationCode(doc: AuthorizationCodeDoc) {
  const db = await getDb();
  await db
    .collection<AuthorizationCodeDoc>("oauth_authorization_codes")
...
export async function deleteAuthorizationCode(code: string) {
  ...
  await db
    ...
    .deleteOne({ _id: code } as any);
}

export async function insertRefreshToken(doc: RefreshTokenDoc) {
  const db = await getDb();
  await db
    .collection<RefreshTokenDoc>("oauth_refresh_tokens")
    .insertOne(doc as any);
  return doc;
}
...
export async function findRefreshToken(hashed: string) {
  ...
  const doc = await db
    .collection<RefreshTokenDoc>("oauth_refresh_tokens")
    .findOne({ hashed_token: hashed, revoked: { $ne: true } } as any);
  return doc || null;
}
...
export async function revokeRefreshToken(id: string) {
  ...
  await db
    ...
    .updateOne({ _id: id } as any, { $set: { revoked: true } });
}
