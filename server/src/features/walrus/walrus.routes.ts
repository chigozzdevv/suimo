...
export async function registerWalrusRoutes(app: FastifyInstance) {
  ...
  r.post(
    ...
    async (req) => {
      ...
      try {
        await sui.getProtocolConfig();
      } catch (e) {
        if (isConnectTimeoutError(e)) {
          rpc = pickFallbackRpc(rpc);
          sui = makeSuiClient(rpc);
        }
      }
      const walrus = new WalrusClient({ network, suiClient: sui as any });
      const { storageCost, writeCost, totalCost } = await walrus.storageCost(
        Number(size_bytes),
        Number(epochs),
      );
      ...
      try {
        ...
        const bytes = await (tx as any).build({
          client: sui,
          onlyTransactionKind: true,
        });
        const dry = await sui.dryRunTransactionBlock({
  ...
  r.post(
    ...
    async (req) => {
      ...
      const walrus = new WalrusClient({ network, suiClient: sui as any });
      const { storageCost, totalCost, writeCost } = await walrus.storageCost(
        Number(size_bytes),
        Number(add_epochs),
      );
      const dec = Number(process.env.WAL_DECIMALS || env.WAL_DECIMALS || 9);
