...
export async function findConnectorById(id: string) {
  ...
  const doc = await db
    .collection<ConnectorDoc>("connectors")
...
export async function listConnectorsByOwner(userId: string) {
  ...
  const cursor = db
    .collection<ConnectorDoc>("connectors")
    .find({ owner_user_id: userId, status: { $ne: "disabled" } } as any)
    .limit(100);
...
export async function updateConnector(
  id: string,
  patch: Partial<Omit<ConnectorDoc, "_id" | "owner_user_id">>,
) {
  const db = await getDb();
  await db
    .collection<ConnectorDoc>("connectors")
    .updateOne({ _id: id } as any, { $set: patch } as any);
}
...
export async function disableConnector(id: string) {
  ...
  await db
    .collection<ConnectorDoc>("connectors")
    .updateOne({ _id: id } as any, { $set: { status: "disabled" } } as any);
}
