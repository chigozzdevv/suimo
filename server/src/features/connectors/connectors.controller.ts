...
} from "@/features/connectors/connectors.schema.js";
import {
  insertConnector,
  listConnectorsByOwner,
  findConnectorById,
  updateConnector,
  disableConnector,
} from "@/features/connectors/connectors.model.js";

export async function createConnectorController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
...
export async function listConnectorsController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
  ...
  return reply.send({
    items: items.map((c) => ({ id: c._id, type: c.type, status: c.status })),
  });
}

export async function getConnectorController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function updateConnectorController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
  ...
  if (body.config)
    patch.enc_config = encryptSecret(JSON.stringify(body.config));
  await updateConnector(id, patch);
  return reply.send({ id });
}

export async function deleteConnectorController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
