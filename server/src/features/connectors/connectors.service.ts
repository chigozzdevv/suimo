...
export async function fetchViaConnector(
  resource: ResourceDoc,
  connector: ConnectorDoc,
) {
  if (
    connector.type === "internal" &&
    (resource.walrus_blob_id || resource.walrus_quilt_id)
  ) {
    return { kind: "internal" as const };
  }
  if (!resource.domain || !resource.path)
    throw new Error("RESOURCE_URL_MISSING");
