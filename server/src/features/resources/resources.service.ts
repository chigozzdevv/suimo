...
export async function createResourceForProvider(
  providerId: string,
  ...
  const doc: ResourceDoc = {
    ...
    policy: {
      ...
      allow: (input as any).allow_agent_ids,
      deny_paths: (input as any).deny_paths,
      modes: input.modes as any,
    },
...
export async function listPublicResourcesByTags(tags: string[], limit = 24) {
  const db = await getDb();
  const safeTags = Array.from(new Set((tags || []).filter(Boolean))).slice(
    0,
    10,
  );
  const filter: any = {
...
export async function listPublicResourcesWithCategory(
  category: string,
  limit = 24,
) {
  const db = await getDb();
...
export async function searchPublicResources(
  query: string,
  opts?: { category?: string; format?: string[]; limit?: number },
) {
  const db = await getDb();
