...
export async function getUserCaps(
  userId: string,
): Promise<SpendingCapsDoc | null> {
  const db = await getDb();
  return (
    (await db
      .collection<SpendingCapsDoc>("spending_caps")
      .findOne({ _id: userId })) || null
  );
}

export async function setUserCaps(
  userId: string,
  caps: Partial<Omit<SpendingCapsDoc, "_id">>,
): Promise<void> {
  const db = await getDb();
  await db
    .collection<SpendingCapsDoc>("spending_caps")
