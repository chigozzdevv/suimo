...
async function getSigningKey() {
  if (!keyPromise) {
    keyPromise = (async () => {
      const { ED25519_PRIVATE_KEY, ED25519_PRIVATE_KEY_PATH } =
        process.env as any;
      ...
      if (ED25519_PRIVATE_KEY) {
        try {
          ...
          console.warn(
            "Failed to parse ED25519_PRIVATE_KEY, falling back to ED25519_PRIVATE_KEY_PATH",
          );
        }
      }
      ...
      throw new Error(
        "ED25519_PRIVATE_KEY or ED25519_PRIVATE_KEY_PATH is required",
      );
    })();
  }
  return keyPromise;
