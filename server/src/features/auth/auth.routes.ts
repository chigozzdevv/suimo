...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  r.post(
    "/forgot_password",
    { schema: { body: forgotPasswordInput } },
    forgotPasswordController,
  );
  r.post(
    "/reset_password",
    { schema: { body: resetPasswordInput } },
    resetPasswordController,
  );
  r.post(
    "/change_password",
    { preHandler: [requireUser], schema: { body: changePasswordInput } },
    changePasswordController,
  );
  r.post(
    "/wallet/challenge",
    { schema: { body: walletChallengeInput } },
    walletChallengeController,
  );
  r.post(
    "/wallet/link",
    { preHandler: [requireUser], schema: { body: walletVerifyInput } },
    walletLinkController,
  );
  r.post(
    "/wallet/login",
    { schema: { body: walletVerifyInput } },
    walletLoginController,
  );
