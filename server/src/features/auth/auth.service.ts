...
export async function signupService(
  ...
  try {
    ...
    if (err?.message === "SERVER_MISCONFIG")
      return { ok: false as const, error: "SERVER_MISCONFIG" };
    throw err;
  }
}
...
export async function createWalletChallenge(address: string, chain: "sui") {
  ...
  await createOrReplaceChallenge({
    _id: "wc_" + randomUUID(),
    chain,
    address,
    nonce,
...
export async function verifyWalletLink(
  ...
  await upsertWalletLink({
    _id: existing?._id || "wl_" + randomUUID(),
    user_id: userId,
    chain,
    address,
