...
export async function findUserById(id: string) {
  const db = await getDb();
  return (
    (await db.collection<UserDoc>("users").findOne({ _id: id } as any)) || null
  );
}
...
export async function updateUserPassword(id: string, password_hash: string) {
  ...
  await db
    ...
    .updateOne({ _id: id } as any, { $set: { password_hash } });
}
...
export async function upsertWalletLink(doc: WalletLinkDoc) {
  const db = await getDb();
  await db
    .collection<WalletLinkDoc>("wallet_links")
    .updateOne(
      { chain: doc.chain, address: doc.address },
      {
        $setOnInsert: { _id: doc._id, created_at: doc.created_at },
        $set: { user_id: doc.user_id, last_verified_at: doc.last_verified_at },
      },
      { upsert: true },
    );
}
...
export async function createOrReplaceChallenge(doc: WalletChallengeDoc) {
  ...
  await db
    ...
    .updateOne(
      ...
      {
        $setOnInsert: { _id: doc._id, created_at: doc.created_at },
        $set: { nonce: doc.nonce, expires_at: doc.expires_at, used: doc.used },
      },
      { upsert: true },
    );
}
...
export async function markChallengeUsed(id: string) {
  ...
  await db
    .collection<WalletChallengeDoc>("wallet_challenges")
    .updateOne({ _id: id }, { $set: { used: true } });
}
...
export async function findValidResetToken(token_hash: string) {
  ...
  return (
    (await db
      .collection<PasswordResetDoc>("password_resets")
      .findOne({ token_hash, used: false, expires_at: { $gt: now } } as any)) ||
    null
  );
}
