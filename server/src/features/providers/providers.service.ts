...
export async function verifySiteInit(userId: string, domain: string, method: 'dns' | 'file') {
  ...
  if (method === 'dns') {
    return { method: 'dns' as const, token, instructions: `Add TXT record: suimo-verify=${token}` };
  }
  return { method: 'file' as const, token, instructions: `Upload file at: https://${domain}/.well-known/suimo.txt with content: ${token}` };
}
...
export async function verifySiteCheck(userId: string, domain: string, method: 'dns' | 'file', token: string) {
  ...
  if (method === 'dns') {
    try {
      ...
      const match = flattened.find((txt) => txt.trim() === `suimo-verify=${token}`);
      if (match) {
        await markSiteVerified(providerId, domain);
        return { verified: true } as const;
      }
