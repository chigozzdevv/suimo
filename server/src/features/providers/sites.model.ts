...
export async function getSiteVerification(
  providerId: string,
  domain: string,
): Promise<SiteVerificationDoc | null> {
  const db = await getDb();
  return (
    (await db
      .collection<SiteVerificationDoc>("site_verifications")
      .findOne({ _id: key(providerId, domain) } as any)) || null
  );
}

export async function getAllProviderDomains(
  providerId: string,
): Promise<SiteVerificationDoc[]> {
  const db = await getDb();
  return await db
    .collection<SiteVerificationDoc>("site_verifications")
...
export async function deleteDomain(providerId: string, domain: string) {
  ...
  await db
    .collection<SiteVerificationDoc>("site_verifications")
    .deleteOne({ _id: key(providerId, domain) });
}

export async function upsertSiteVerification(
  doc: Omit<SiteVerificationDoc, "_id" | "created_at" | "status"> & {
    status?: SiteVerificationDoc["status"];
  },
) {
  const db = await getDb();
  ...
  await db
    .collection<SiteVerificationDoc>("site_verifications")
    .updateOne({ _id }, { $set: toSet }, { upsert: true });
