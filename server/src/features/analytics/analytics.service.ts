...
export async function getProviderEarningsTimeSeries(
  providerId: string,
  days = 30,
) {
  const db = await getDb();
...
export async function getConsumerRecentActivity(userId: string, limit = 5) {
  ...
  const resourceIds = Array.from(
    new Set(requests.map((r) => r.resource_id).filter(Boolean)),
  );
  let resourceMap: Record<string, { title?: string; domain?: string }> = {};
  if (resourceIds.length) {
    const resources = await db
      .collection("resources")
      .find({ _id: { $in: resourceIds } } as any)
      .toArray();
    resourceMap = Object.fromEntries(
      resources.map((r: any) => [r._id, { title: r.title, domain: r.domain }]),
    );
  }
...
export async function getConsumerTopAgents(userId: string, limit = 5) {
  ...
  if (agentIds.length) {
    const agents = await db
      .collection("agents")
      .find({ _id: { $in: agentIds } } as any)
      .toArray();
    agentMap = Object.fromEntries(
      agents.map((agent: any) => [agent._id, { name: agent.name }]),
    );
  }
...
export async function getConsumerTopSources(userId: string, limit = 5) {
  ...
  return rows.map((row) => ({
    resource_id: row._id,
    resource_title:
      resourceMap[row._id]?.title || row._id || "Unknown resource",
...
export async function getProviderResourcePerformance(
  providerId: string,
  limit = 10,
) {
  const db = await getDb();
  ...
  const resourceMap = Object.fromEntries(
    resources.map((r: any) => [String(r._id), r]),
  );
  const requestMap = Object.fromEntries(
    requestStats.map((s: any) => [String(s._id), s]),
  );
  const impressionMap = Object.fromEntries(
    impressionStats.map((s: any) => [s._id, s]),
  );
  ...
  const performance = resourceIds.map((rid) => {
    ...
    const selectionRate =
      impStat.impressions > 0 ? impStat.selections / impStat.impressions : 0;
  ...
  return performance.sort((a, b) => b.earned - a.earned).slice(0, limit);
}
