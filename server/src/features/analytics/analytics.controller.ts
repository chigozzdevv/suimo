...
import {
  ...
  getProviderEarningsTimeSeries,
  getConsumerRecentActivity,
  getConsumerTopAgents,
  getConsumerTopSources,
  getProviderResourcePerformance,
...
const daysQuery = z
  ...
  .partial();
const limitQuery = z
  .object({ limit: z.coerce.number().int().positive().max(50).default(5) })
  .partial();

export async function getUserSpendingController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
...
export async function getProviderEarningsController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function getProviderSearchController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function getConsumerActivityController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function getConsumerTopAgentsController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function getConsumerTopSourcesController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function getProviderResourcePerformanceController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
