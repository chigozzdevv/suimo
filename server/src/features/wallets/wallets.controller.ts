...
import {
  ...
  listDepositsService,
} from "@/features/wallets/wallets.service.js";
import {
  getPin,
  setNewPin,
  changePin,
  remainingAttempts,
} from "@/features/wallets/pin.model.js";

export async function getWalletsController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
...
const depositInput = z.object({
  role: z.enum(["payer", "payout"]).default("payer"),
...
export async function createDepositController(
  ...
  try {
    ...
    if (
      err?.message === "AMOUNT_EXCEEDS_LIMIT" ||
...
const withdrawalInput = z.object({
  role: z.enum(["payer", "payout"]).default("payout"),
  amount: z.number().positive(),
  to: z.string().min(10),
...
export async function createWithdrawalController(
  ...
  try {
    ...
    if (
      ["PIN_REQUIRED", "PIN_NOT_SET", "PIN_INVALID", "PIN_LOCKED"].includes(
        String(err?.message),
      )
    ) {
...
export async function getPinStatusController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
...
export async function setPinController(
  req: FastifyRequest,
  reply: FastifyReply,
) {
  const userId = (req as any).userId as string;
  ...
  try {
    ...
    const code = ["PIN_ALREADY_SET", "PIN_FORMAT_INVALID"].includes(msg)
      ? 400
      : 500;
