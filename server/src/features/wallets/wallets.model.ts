...
export async function getOrInitWallet(
  userId: string,
  role: "payer" | "payout",
) {
...
export async function createHold(
  ...
  await db
    ...
    .updateOne(
      { _id: payer._id },
      { $inc: { available: -amount, blocked: amount } },
    );
  const hold: HoldDoc = {
    _id: randomUUID(),
    wallet_id: payer._id,
...
export async function releaseHold(holdId: string) {
  ...
  await db
    ...
    .updateOne(
      { _id: wallet._id },
      { $inc: { available: hold.amount, blocked: -hold.amount } },
    );
  await db
    .collection<HoldDoc>("holds")
    .updateOne({ _id: holdId }, { $set: { state: "released" } });
}
...
export async function captureHold(
  ...
  ops.push({
    updateOne: {
      filter: { _id: payerWallet._id },
      update: {
        $inc: { blocked: -hold.amount, available: Math.max(0, delta) },
      },
    },
  });
  ...
  ops.push({
    updateOne: {
      filter: { _id: providerPayout._id },
      update: { $inc: { available: finalAmount - feeAmount } },
    },
  });
  ...
  if (feeAmount > 0) {
    ...
    ops.push({
      updateOne: {
        filter: { _id: feeWallet._id },
        update: { $inc: { available: feeAmount } },
      },
    });
  }
  ...
  await db
    .collection<HoldDoc>("holds")
    .updateOne({ _id: holdId }, { $set: { state: "captured" } });
  ...
  const entries: LedgerDoc[] = [
    {
      ...
      amount: finalAmount,
      ref_type: "capture",
      ref_id: hold.request_id,
      ts: now,
    } as any,
    {
      _id: randomUUID(),
      wallet_id: providerPayout._id,
      type: "credit",
      amount: finalAmount - feeAmount,
      ref_type: "capture",
      ref_id: hold.request_id,
      ts: now,
    } as any,
...
export async function creditWallet(
  ...
  await db
    .collection<WalletDoc>("wallets")
    .updateOne({ _id: w._id }, { $inc: { available: amount } });
  await db
...
export async function listWalletsByOwnerUserId(userId: string) {
  ...
  return db
    .collection<WalletDoc>("wallets")
    .find({ owner_user_id: userId } as any)
    .toArray();
}

export async function listLedgerEntriesByWalletIds(
  walletIds: string[],
  limit = 100,
) {
  const db = await getDb();
