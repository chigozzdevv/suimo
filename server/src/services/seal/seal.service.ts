import { getAllowlistedKeyServers, retrieveKeyServers, KeyStore, SessionKey, encrypt as sealEncrypt, AesGcm256, EncryptedObject } from '@mysten/seal';
...
export async function sealEncryptForPolicy(plaintext: Uint8Array, _policyId: string): Promise<{ ciphertextEnvelope: Uint8Array; meta: { algo: string; size_bytes: number } }> {
  const env = loadEnv();
  ...
  const keyServers = await retrieveKeyServers({ objectIds: keyServerIds, client: suiClient() as any });
  ...
  const packageId = Uint8Array.from(Buffer.from(policyPackageIdHex.replace(/^0x/, ''), 'hex'));
  // For identity, use the resource id or server-generated id; placeholder zeros (caller should supply)
  const id = new Uint8Array(32);
  const { encryptedObject } = await sealEncrypt({ keyServers, threshold: Math.min(3, keyServers.length), packageId, id, encryptionInput });
  return { ciphertextEnvelope: encryptedObject, meta: { algo: 'seal-ibe-aes256gcm', size_bytes: plaintext.byteLength } };
}

export async function sealDecryptForAccess(cipher: { chunks: string[] }, _policyId?: string, _ctx?: { requestId?: string }): Promise<{ chunks: string[]; bytes: number }> {
  const env = loadEnv();
  const network = (env.WALRUS_NETWORK === 'mainnet' ? 'mainnet' : 'testnet') as 'mainnet' | 'testnet';
  const keyServerIds = getAllowlistedKeyServers(network);
  const keyServers = await retrieveKeyServers({ objectIds: keyServerIds, client: suiClient() as any });
  ...
  const bytes = Buffer.concat(cipher.chunks.map((b64) => Buffer.from(b64, 'base64')));
  ...
  await ks.fetchKeys({ keyServers, threshold: Math.min(3, keyServers.length), packageId, ids: [id], txBytes, sessionKey: session });
