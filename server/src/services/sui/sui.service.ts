...
function platformKeypair() {
  ...
  try {
    ...
    throw new Error(
      `Invalid SUI_PLATFORM_PRIVATE_KEY format: ${e?.message || e}`,
    );
  }
}
...
export async function transferWal(
  toAddress: string,
...
export async function topUpSui(
  toAddress: string,
  amountSui: number,
): Promise<string> {
  const kp = platformKeypair();
  ...
  const [coin] = tx.splitCoins(tx.gas, [
    tx.pure.u64(Math.round(amountSui * 1e9)),
  ]);
