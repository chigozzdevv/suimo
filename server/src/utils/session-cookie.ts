...
const COOKIE_MAX_AGE = Number(
  process.env.SESSION_COOKIE_MAX_AGE || 60 * 60 * 24 * 7,
);
...
export function buildSessionCookie(value: string, host?: string) {
  const includeDomain =
    hostMatchesCookieDomain(host) || (!host && !!COOKIE_DOMAIN);
  const parts = [
    `${COOKIE_NAME}=${value}`,
    ...buildCommonCookieParts(includeDomain),
  ];
  return parts.join("; ");
}

export function buildSessionCookieForHost(
